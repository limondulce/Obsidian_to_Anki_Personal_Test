...
abstract class AbstractFile {
  ...
  setup_frozen_fields_dict() {
    ...
    for (let match of this.file.matchAll(this.data.FROZEN_REGEXP)) {
      ...
      const parsed_fields: Record<string, string> = new Note(virtual_note, this.data.fields_dict, this.data.curly_cloze, this.data.highlights_to_cloze, this.formatter).getFields();
    ...
    for (let search_id of [true, false]) {
      for (let search_tags of [true, false]) {
        ...
        for (let match of findignore(regexp, this.file, this.ignore_spans)) {
          ...
          const parsed: AnkiConnectNoteAndID = new RegexNote(match, note_type, this.data.fields_dict, search_tags, search_id, this.data.curly_cloze, this.data.highlights_to_cloze, this.formatter).parse(this.target_deck, this.url, this.frozen_fields_dict, this.data, this.data.add_context ? this.getContextAtIndex(match.index) : "");
          if (search_id) {
